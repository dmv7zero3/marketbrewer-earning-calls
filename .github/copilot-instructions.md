...
- **Static React SPA:** Built with React 18, TypeScript, React Router v6. Entry point at `src/index.tsx`.
...
- **Seeking Alpha Scraper (CLI):** scripts/scraping includes scraper + parser + validators + audit logging (can persist to DynamoDB via CLI; not wired to API ingest yet).
...
# Seeking Alpha (scraper uses cookies stored in scraper-cookies.json)
...
│   └── scraping/              # Seeking Alpha scraper (CLI)
│       ├── scraper.ts         # Puppeteer automation
│       ├── parser.ts          # HTML extraction
│       ├── validators/        # Validation + confidence
│       └── audit/             # Audit logging
├── infrastructure/
